...
class GetStatisticsSchema(Schema):
    ...
    def test_invalid_language(self):
        ...
        self.assertEqual(response.data, b'"Only en language code is supported"\n')  # expected output

    def test_missing_title(self):
        response = self.app.get('/get_article_statistics?lang=en&site=wikipedia')
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.data, b'{"error": "{\'title\': [\'Missing data for required field.\']}"}\n')

    def test_invalid_site(self):
        response = self.app.get('/get_article_statistics?lang=en&site=example.com&title=Test')
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.data, b'"Only \'wikipedia\' site is supported"\n')  # expected output
