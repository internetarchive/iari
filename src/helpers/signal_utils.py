...
from src.constants.api_constants import API_WIKI_SIGNALS, SIGNALS_DOCS_PROPERTY, SIGNALS_DOCS_FILENAME

SIGNALS_CACHE_DIR = f"{config.iari_cache_dir}"
SIGNALS_CSV = config.iari_signals_csv


def get_signal_docs(force_refresh=False):
    """
    returns signal doc JSON from Wiki Signal API
    """
    if force_refresh == False and is_cached(SIGNALS_DOCS_FILENAME, CacheType.default):
        doc_json = get_cache("signal_docs.json", CacheType.default)
        return {
            "retrieved_from_cache": True,
            SIGNALS_DOCS_PROPERTY: doc_json
        }
    
    ### api_wiki_signals = "https://wikisignals.org/wp-json/publications/v1/domain/"
    endpoint = f"{API_WIKI_SIGNALS}domain/?view=definitions"
    ...
    if response.status_code == 200:
        ...
        # save sig_docs in cache
        set_cache(SIGNALS_DOCS_FILENAME, CacheType.default, payload=sig_docs)

        # return signal docs
        return {
            SIGNALS_DOCS_PROPERTY : sig_docs
        }
    else:
        return {
            SIGNALS_DOCS_PROPERTY: {}
        }
...
def get_signal_data_for_domain(domain, force_refresh=False):
    ...
    ### api_wiki_signals = API_WIKI_SIGNALS
    endpoint = f"{API_WIKI_SIGNALS}domain/?name={domain}"
