...
class Properties(WcdBaseModel):
    ...
    def get_all_property_names(self) -> List[str]:
        ...
        return [str(wikibase_property_name) for wikibase_property_name in self.properties]

    def get_all_property_labels(self) -> List[str]:
        """Get all labels of properties formatted with space and titlecase"""
        return [str(wikibase_property_label.replace("_", " ").title()) for wikibase_property_label in self.properties]
