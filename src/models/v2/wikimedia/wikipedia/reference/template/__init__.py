...
class WikipediaTemplateV2(BaseModel):

    parameters: OrderedDict = OrderedDict()
    raw_template: Template  # Union[Template, str]  # We allow union here to enable easier testing
    ...
    def __add_template_name_to_parameters__(self):
        ...
        # FIXME should not save template name in parameters -
        #   save at top level of template properties
    ...
    def __fix_class_key__(self):
        ...
        for key in self.parameters:
            if (
                key == "class"
            ):  # NB for some reason this changes "class" key to "news_class"
                myDict["news_class"] = self.parameters[key]
            else:
                myDict[key] = self.parameters[key]
        self.parameters = myDict
        ...
        for key in self.parameters:
            ...
            for replacement in replacements:
                if replacement == key:
                    ...
                    myDict[new_key] = self.parameters[key]
