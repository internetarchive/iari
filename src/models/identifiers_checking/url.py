...
class Url(WikipediaUrl):
    ...
    testdeadlink_response: Optional[Dict[str, Any]] = None
    # timeout: int = 2
    ...
    #     """We decided to use these headers because of
    #     https://github.com/internetarchive/wari/issues/698"""
    ...
    def __check_url_with_testdeadlink_api__(self):
        ...
        if not config.testdeadlink_key:
            ...
            if response.status_code == 200:
                self.testdeadlink_response = response.json()
                # print(data)
                # exit()
                # TODO deprecate this, testdeadlink should
                #  respond in a way that is nice for downstream data consumers like IARE
                #  by default
                # Attempt to parse the result
                if "results" in self.testdeadlink_response:
                    # array of results
                    results = self.testdeadlink_response["results"]
                    # Get the status code
